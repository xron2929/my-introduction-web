    ...
    */
    ...
    function addArea()
    {
        ...
        cbmake.dataset.type="checkboxContent";  
        //checkboxContent추가
    ...
    function endedit()
    {
        const checkboxgrouping=document.querySelectorAll('[data-type=checkboxContent]');
        for(var name in checkboxgrouping)   //for(var name:checkboxgroping)
        {
            if(checkboxgrouping[name].checked===true) //체크박스가 체크되어있을 때
            {
                ...
                checkboxgrouping[name].parentNode.className="checkboxGroup";
                //새로 생긴 영역도 탐색할 수 있도록 checkboxGroup 클래스에 추가해줌
    ...
    //first child,last child 이렇게 노드로 탐색해도 어차피 최종본에는 주석을 다 지우고 minify (경량화)로 지우기 떄문에
    //사용함